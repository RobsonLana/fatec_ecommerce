<?php
    ...
    if ($client) {
        $client = get_client_address($connection, $client);
        $client = $client[0];

        $client_name = $client['nome_cli'];

        $head_bar_title = $head_bar_title . " - " . $client_name;

        $orders = list_client_orders($connection, $client['cpf_cnpj_cli']);
    } else {
...
<html>
    ...
    <body>
        <?=header_bar($head_bar_title, 'admin', $referer_url)?>
        <?php
            if (count($orders) > 0) {
        ?>
                    ...
                    <li class="product_card">
                        ...
                        <div class="buttons">
                            <form method="GET" action="./orders.php">
                                <input type="hidden" name="client" value="<?=$order['cpf_cnpj_cli']?>">
                                <button><?= $client_name ? $client_name : $order['nome_cli']?></button>
                            </form>
                            <form method="GET" action="./order_details.php">
                                <input type="hidden" name="order_id" value="<?=$order['numero_compra']?>">
                                <button>Detalhes</button>
                            </form>
                            <form method="GET" action="../page/invoice.php">
                                <input type="hidden" name="order_id" value="<?= $order['numero_compra']?>">
